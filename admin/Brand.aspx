<%@ Page Title="Brand" Language="C#" MasterPageFile="~/admin/admin.master" AutoEventWireup="true" CodeFile="Brand.aspx.cs" Inherits="admin_Brand " %>
        ...
        <div>
            <div class="container" ng-app="app" ng-controller="TodoController">
                    ...
                    <table id="lista-compras" class="table table-striped">
                        <thead>
                            <tr>
                                ...
                                <th ng-click="sortBy('brand_id')" style="text-decoration: underline; cursor: pointer;">Brand ID</th>
                                <th ng-click="sortBy('brand_name')" style="text-decoration: underline; cursor: pointer;">Brand Name</th>
                        ...
                        <tbody>
                            <tr ng-repeat="item in filteredItems | orderBy:sortField:reverseOrder | filter:searchValue | startFrom:(currentPage - 1) * numPerPage | limitTo:numPerPage" class="comprado-{{ item.brand_name }}">
                                <td><input type='checkbox' ng-checked="checkall" id="item-{{ item.brand_id }}" value="{{ item.brand_id }}"></td>
                                <td><strong>{{ item.brand_id }}</strong></td>
                                <td>{{ item.brand_name }}</td>
                                <td><div style="width: 100%; height: 50px; overflow-y:auto;">{{ item.brand_description }}</div></td>
                                <td>{{ item.images }}</td>
                                <td>{{ item.create_date | filterdate | date:'dd/MM/yyyy hh:mm:ss' }}</td>
                                <td>
                                    <a class="btn btn-warning btn-small" ng-click="editarItem(item.brand_id)">
                                        <i class="icon-pencil icon-white"></i>Edit
                                    </a>
                                    <a class="btn btn-danger btn-small" ng-click="deleteItem(item.brand_id)">
                ...
                <table style="width: 100%">
                    <thead>
                        <tr>
                            <th colspan="2">Brand form</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            <td>Brand ID:<span style="color:red">*</span></td>
                            <td><input type="number" ng-model="item.brand_id" placeholder="Brand ID" style="width: 500px;" required></td>
                        </tr>
                        <tr>
                            <td>Brand Name:<span style="color:red">*</span></td>
                            <td><input type="text" ng-model="item.brand_name" placeholder="Brand Name" required style="width: 500px;"></td>
                        </tr>
                        <tr>
                            <td>Brand Description: </td>
                            <td><textarea type="text" ng-model="item.brand_description" placeholder="Brand Description" class="input-small" cols="69" rows="10"></textarea></td>
                        </tr>
                        <tr>
                            <td>Images: </td>
            ...
            <script src="AngularJS/app/brand.js"></script>
